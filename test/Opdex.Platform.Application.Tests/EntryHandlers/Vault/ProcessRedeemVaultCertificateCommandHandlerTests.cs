...
namespace Opdex.Platform.Application.Tests.EntryHandlers.Vault
{
    public class ProcessRedeemVaultCertificateCommandHandlerTests
    {
        ...
        public async Task Handle_MediatorMakeRedeemVaultCertificateCommand_Send()
        {
            ...
            // Act
            await _handler.Handle(new ProcessRedeemVaultCertificateCommand(walletName, walletAddress, walletPassword,
                                                                           vault, holder), cancellationToken);

            // Assert
            _mediatorMock.Verify(callTo => callTo.Send(
                It.Is<MakeRedeemVaultCertificateCommand>(command => command.WalletName == walletName
        ...
        public async Task Handle_MediatorPersistMarketPermissionCommand_Return()
        {
            ...
            // Act
            var response = await _handler.Handle(new ProcessRedeemVaultCertificateCommand(walletName, walletAddress,
