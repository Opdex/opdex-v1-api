...
namespace Opdex.Platform.Domain.Tests.Models.Pools.Snapshots
{
    public class VolumeSnapshotTests
    {
        ...
        [InlineData("1.234")]
        [InlineData(null)]
        public void CreateVolumeSnapshot_InvalidVolumeCrs_ThrowsArgumentOutOfRangeException(string volumeCrs)
        {
            // Arrange
            // Act
            void Act() => new VolumeSnapshot(volumeCrs, "123", 4.56m);

            // Assert
            Assert.Throws<ArgumentOutOfRangeException>(Act).Message.Should().Contain($"{nameof(volumeCrs)} must be a numeric value.");
        }
