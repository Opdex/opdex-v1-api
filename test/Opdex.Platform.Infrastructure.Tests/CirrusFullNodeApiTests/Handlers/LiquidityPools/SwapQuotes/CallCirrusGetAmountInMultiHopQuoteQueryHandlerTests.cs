...
{
    public class CallCirrusGetAmountInMultiHopQuoteQueryHandlerTests
    {
        private readonly Mock<ISmartContractsModule> _smartContractModule;
        private readonly CallCirrusGetAmountInMultiHopQuoteQueryHandler _handler;

        public CallCirrusGetAmountInMultiHopQuoteQueryHandlerTests()
        {
            _smartContractModule = new Mock<ISmartContractsModule>();

            _handler = new CallCirrusGetAmountInMultiHopQuoteQueryHandler(_smartContractModule.Object);
        }
        ...
        {
            // Arrange
            var query = new CallCirrusGetAmountInMultiHopQuoteQuery(new Address("tMCT728cPmhexrrqqkErDbYAC9eA1wNGZA"),
            ...
            {
                new SmartContractMethodParameter(query.TokenOutAmount),
            ...
            _smartContractModule.Verify(callTo => callTo.LocalCallAsync(It.Is<LocalCallRequestDto>(request => request.ContractAddress == query.Router
                                                                                                           ...
                                                                                                           && request.MethodName == "GetAmountIn"
