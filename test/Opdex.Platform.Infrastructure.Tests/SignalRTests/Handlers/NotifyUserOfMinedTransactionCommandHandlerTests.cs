...
public class NotifyUserOfMinedTransactionCommandHandlerTests
{
    ...
    {
        // Arrange
        var request = new NotifyUserOfMinedTransactionCommand("PMU9EjmivLgqqARwmH1iT1GLsMroh6zXXN", new Sha256(2839832439898423984));

        // Act
        await _handler.Handle(request, CancellationToken.None);

        // Assert
        _hubContextMock.Verify(callTo => callTo.Clients.User(request.User.ToString()).OnTransactionMined(request.TxHash.ToString()), Times.Once);
    }
}