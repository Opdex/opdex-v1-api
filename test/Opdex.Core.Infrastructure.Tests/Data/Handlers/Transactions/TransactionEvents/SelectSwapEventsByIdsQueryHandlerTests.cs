...
{
    public class SelectSwapEventsByIdsQueryHandlerTests
    {
        private readonly Mock<IDbContext> _dbContext;
        private readonly SelectSwapEventsByIdsQueryHandler _handler;
        
        public SelectSwapEventsByIdsQueryHandlerTests()
        {
            var mapper = new MapperConfiguration(config => config.AddProfile(new CoreInfrastructureMapperProfile())).CreateMapper();
            
            _dbContext = new Mock<IDbContext>();
            _handler = new SelectSwapEventsByIdsQueryHandler(_dbContext.Object, mapper);
        }
        ...
        public async Task SelectSwapEventByTransactionId_Success()
        {
            ...
            var command = new SelectSwapEventsByIdsQuery(new [] { new TransactionEventSummary(1,1,1,1,1,"1") });
