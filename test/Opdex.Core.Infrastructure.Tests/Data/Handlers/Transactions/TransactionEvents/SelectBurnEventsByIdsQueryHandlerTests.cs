...
{
    public class SelectBurnEventsByIdsQueryHandlerTests
    {
        private readonly Mock<IDbContext> _dbContext;
        private readonly SelectBurnEventsByIdsQueryHandler _handler;
        
        public SelectBurnEventsByIdsQueryHandlerTests()
        {
            var mapper = new MapperConfiguration(config => config.AddProfile(new CoreInfrastructureMapperProfile())).CreateMapper();
            
            _dbContext = new Mock<IDbContext>();
            _handler = new SelectBurnEventsByIdsQueryHandler(_dbContext.Object, mapper);
        }
        ...
        public async Task SelectBurnEventByTransactionId_Success()
        {
            ...
            var command = new SelectBurnEventsByIdsQuery(new [] { new TransactionEventSummary(1,1,1,1,1,"1") });
