...
{
    public class SelectMintEventsByIdsQueryHandlerTests
    {
        private readonly Mock<IDbContext> _dbContext;
        private readonly SelectMintEventsByIdsQueryHandler _handler;
        
        public SelectMintEventsByIdsQueryHandlerTests()
        {
            var mapper = new MapperConfiguration(config => config.AddProfile(new CoreInfrastructureMapperProfile())).CreateMapper();
            
            _dbContext = new Mock<IDbContext>();
            _handler = new SelectMintEventsByIdsQueryHandler(_dbContext.Object, mapper);
        }
        ...
        public async Task SelectMintEventByTransactionId_Success()
        {
            ...
            var command = new SelectMintEventsByIdsQuery(new [] { new TransactionEventSummary(1,1,1,1,1,"1") });
