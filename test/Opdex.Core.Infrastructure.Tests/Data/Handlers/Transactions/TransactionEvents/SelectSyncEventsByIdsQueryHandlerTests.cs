...
{
    public class SelectSyncEventsByIdsQueryHandlerTests
    {
        private readonly Mock<IDbContext> _dbContext;
        private readonly SelectSyncEventsByIdsQueryHandler _handler;
        
        public SelectSyncEventsByIdsQueryHandlerTests()
        {
            var mapper = new MapperConfiguration(config => config.AddProfile(new CoreInfrastructureMapperProfile())).CreateMapper();
            
            _dbContext = new Mock<IDbContext>();
            _handler = new SelectSyncEventsByIdsQueryHandler(_dbContext.Object, mapper);
        }
        ...
        public async Task SelectSyncEventByTransactionId_Success()
        {
            ...
            var command = new SelectSyncEventsByIdsQuery(new [] { new TransactionEventSummary(1,1,1,1,1,"1") });
