...
{
    public static IRuleBuilderOptions<T, TEnum> MustBeValidEnumValueOrDefault<T, TEnum>(this IRuleBuilder<T, TEnum> ruleBuilder) where TEnum : Enum
    {
        return ruleBuilder.Must(value => value.Equals(default(TEnum)) || value.IsValid()).WithMessage("Value must be valid for the the enumeration values, or default.");
    }
