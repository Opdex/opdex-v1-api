...
/// <summary>
/// Command to quote withdrawing a pledge to a vault proposal.
/// </summary>
public class CreateWithdrawVaultProposalPledgeQuoteCommand : BaseTransactionQuoteCommand
{
    /// <summary>
    /// Creates a command to quote withdrawing a pledge to a vault proposal.
    /// </summary>
    ...
    public CreateWithdrawVaultProposalPledgeQuoteCommand(Address vault, ulong proposalId, FixedDecimal amount, Address walletAddress) : base(walletAddress)
    {
        ...
        if (amount == FixedDecimal.Zero) throw new ArgumentOutOfRangeException(nameof(amount), "Pledge withdrawal amount must be greater than zero.");
        Vault = vault;
