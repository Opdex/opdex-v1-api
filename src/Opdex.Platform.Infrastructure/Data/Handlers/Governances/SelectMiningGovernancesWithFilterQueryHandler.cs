...
namespace Opdex.Platform.Infrastructure.Data.Handlers.Governances
{
    public class SelectMiningGovernancesWithFilterQueryHandler : IRequestHandler<SelectMiningGovernancesWithFilterQuery, IEnumerable<MiningGovernance>>
    {
        ...
        {
            var governanceId = request.Cursor.Pointer;

            var queryParams = new SqlParams(governanceId, request.Cursor.MinedToken);
        ...
        private static string QueryBuilder(SelectMiningGovernancesWithFilterQuery request)
        {
            ...
            if (!request.Cursor.IsFirstRequest)
            {
                ...
                whereFilter += $" WHERE g.{nameof(MiningGovernanceEntity.Id)} {sortOperator} @{nameof(SqlParams.GovernanceId)}";
            }
