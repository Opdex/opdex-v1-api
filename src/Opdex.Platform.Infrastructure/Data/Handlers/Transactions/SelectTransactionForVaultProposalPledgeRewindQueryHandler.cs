...
public class SelectTransactionForVaultProposalPledgeRewindQueryHandler
    : IRequestHandler<SelectTransactionForVaultProposalPledgeRewindQuery, Transaction>
{
    ...
    private static readonly string SqlQuery =
        @$"SELECT
            ...
            WHERE t.`{nameof(TransactionEntity.From)}` = @{nameof(SqlParams.Pledger)} AND
    ...
    public SelectTransactionForVaultProposalPledgeRewindQueryHandler(IDbContext context, IMapper mapper)
    {
        _context = context ?? throw new ArgumentNullException(nameof(context));
        _mapper = mapper ?? throw new ArgumentNullException(nameof(mapper));
    }

    public async Task<Transaction> Handle(SelectTransactionForVaultProposalPledgeRewindQuery request, CancellationToken cancellationToken)
    {
        var query = DatabaseQuery.Create(SqlQuery, new SqlParams(request.Vault, request.Pledger, request.ProposalPublicId), cancellationToken);
    ...
    {
        internal SqlParams(Address vault, Address pledger, ulong proposalId)
        {
            Vault = vault;
            Pledger = pledger;
        ...
        {
            (uint)TransactionLogType.VaultProposalPledgeLog,
            (uint)TransactionLogType.VaultProposalWithdrawPledgeLog
        };
    }
}
